...
| 工程健康 | 单元测试 104/104 ✅；卫生扫描 121 文件 0 异常 ✅ | `tests/` [8] `scripts/hygiene_scan.py` [11] |
...
表 A headroom 分解（11 配置 × 3 seed，池化）:

| 口径 | U_gf（最优固定策略） | U_cf（逐配置最优） | Headroom-H0 | Headroom-H1 | 判定 |
|---|---|---|---|---|---|
| 池化 / 分格 | （待 EXP-004a 回填） | （待 EXP-004a 回填） | （待回填） | （待回填） | （待回填） |
...
| 门禁 | 命令 | 现状 |
|---|---|---|
| 单元测试 | `python -m pytest tests/ -q` | 104/104（14 个测试文件）[8] |
