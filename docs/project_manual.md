...
| 工程健康 | 单元测试 118/118 ✅（pytest，16 文件）；卫生扫描 136 文件 0 异常 ✅ | `tests/` [8] `scripts/hygiene_scan.py` [11] |
| 工程完成度 | E0 ✅ · E1 🟡（余 sqlite 变体）· E2 🟡 闸门就绪 · E3/E4 🗺️ · E5 持续 ✅ | [1] |
| 研究进度 | PHASE 0/1/1b ✅ · PHASE 5 进行中（EXP-004a 已跑 G1 PASS） | [17] |
...
协议层（ADR-0001 [13]，状态 Accepted ✅）: Corpus / Embedder / Index 三协议已落地（E1），恒等适配器 + 行为等价证明已入盘（`research/results/` 下 `PROOF-E1-EQUIV-*.json`）；sqlite 持久化语料变体遗留。
...
&gt; 人话版：先给一个 30 秒演示，再给三个已完成实验的结论卡；EXP-004a 已跑出 G1 PASS、EXP-005 未运行——两者都留了带冻结表头的空表，跑完填数即可。
...
### 5.5 EXP-004 自适应策略选择 🟡（预注册冻结 [18]；004a 已运行 G1 PASS，004b/c 待启动）
...
度量口径（预注册）: 有序路径恢复率 = 检索结果诱导有向图中 ground truth 链的最长有序子路径长 / L（L=链内事件数）；与既有 chain_connectivity（事后建图层、无向成对连通、EXP-002 的 0.940 来源）**不同层、不同定义，不可互换** [20]。
...
| 门禁 | 命令 | 现状 |
|---|---|---|
| 单元测试 | `python -m pytest tests/ -q` | 118/118（16 个测试文件）[8] |
| 仓库卫生 | `python scripts/hygiene_scan.py` | 136 文件 0 异常（零宽/实体/拆词/幽灵路径/杂散反引号）[11] |
...
| E1 | 检索核心抽象化（Corpus/Embedder/Index 三协议 [13]） | 🟡 协议层已落地 + 行为等价已证（sqlite 持久化语料变体遗留） |
| E2 | 结构信号参与检索（引用扩张通道 + 计数器落地 + 有序路径度量黄金值单测） | 🟡 闸门就绪（H-004/EXP-005 已预注册 [20]）；cite 通道 + 有序度量已落地，B'/C' + 运行器待做 |
| E3 | 记忆控制面 | 🗺️ |
| E4 | 能力接口与权限分级 | 🗺️ |
...
PHASE 0（仓库引导）✅ → PHASE 1（动态检索原型）✅ 首轮 → PHASE 1b（假设修订 EXP-002）✅ → **PHASE 5（自适应搜索策略，进行中: EXP-004a G1 PASS）** → PHASE 6（个人记忆）→ PHASE 8（多智能体编排）→ …；PHASE 9–11（仿生/世界模型/物理接口）已降级为长期目标（无验收、不排期）。

### 8.3 下一步（按优先级）

1. E2 剩余前置（EXP-005 运行的先决条件）: B'/C' 策略变体（启用引用扩张的 B/C）+ EXP-005 运行器（cite 通道与有序路径度量已落地 [19]）；
2. EXP-004b 状态可测性（f1/f2/f3 特征检验，004a 已 G1 PASS 解锁）→ 004c 控制器；
3. 工程债清偿排期（§6.4 三项）+ E1 sqlite 持久化语料变体遗留。
